...
namespace DMZ.UI.Generic
{
    public static class Util
    {
        ...
        private static void NewMethod(Form w, Form mdiParentForm)
        {
            ...
            if (mdiParentForm is DemoMdi)
            {
                if (w is FrmClasse)
                {
                    if (((FrmClasse)w).Controlacesso)
                    {
                        var frmName = ((FrmClasse)w).Name;
                        if (Pbl.Usracessos != null)
                        {
                            ((FrmClasse)w).Usracessos = Pbl.Usracessos.AsEnumerable().FirstOrDefault(xx => xx.Ecran.Equals(frmName));
                        }
                    }
                }
            }
            if (w is FrmClasse)
            {
                if (((FrmClasse)w).Controlacesso)
                {
                    if (((FrmClasse)w).Usracessos!=null)
                    {
                        if (!((FrmClasse)w).Usracessos.Ver)
                        {
                            MsBox.Show("Desculpa não tem acesso!. Consulte o Administrador..");
                            w.Dispose();
                        }
                        else
                        {
        ...
        public static byte[] GetImagem(Image image, bool whitepicture)
        {
            MemoryStream ms;
            if (image!=null)
            {
