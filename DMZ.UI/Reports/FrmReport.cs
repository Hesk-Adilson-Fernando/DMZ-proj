...
namespace DMZ.UI.Reports
{
    public partial class FrmReport : FrmClasse2
    {
        ...
        public void enviarEmailToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        public void SavePDF(bool email = true)
        {
            var pft = Application.StartupPath;
            pft += $"\\PDFFiles";
            if (Directory.Exists(pft))
            {
                reportViewer1.RefreshReport();
                reportViewer1.LocalReport.Print(false, email);
            }
            else
            {
                Directory.CreateDirectory(pft);
                if (Directory.Exists(pft))
                {
                    reportViewer1.RefreshReport();
                    reportViewer1.LocalReport.Print(false, email);
