        ...
        private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (aAnexosEmail.Count > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(txtEnviarPara.Text))
                    {
                        var dt = SQL.GetGenDT("select Email from fnc where Email<>''");
                        if (dt.Rows.Count==0)
                        {
                            MessageBox.Show("Impossível enviar Mensagem sem destinatários", "Aviso do sistema", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            var email = dt.Rows[i][0].ToString();
                             ...
                             //   aAnexosEmail, txtSenhaRemet.Text);
                        }
                        ret = "Mensagem enviada para " + ret + " às " +
                              DateTime.Now.ToString("HH:mm:ss") + " do dia " + DateTime.Now.ToString("dd-MM-yyyy") + ".";
                    }
                    else
                    {

                        //ret = EnviarEmail.EnviaMensagemComAnexos(txtEnviarPara.Text,
                        //    txtEnviadoPor.Text, txtAssunto.Text, txtMensagem.Text,
                        //    aAnexosEmail, txtSenhaRemet.Text);
                    }
                    MessageBox.Show(ret, "Aviso do sistema", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    string ret = string.Empty;
                    ...
                    {
                        //ret = EnviarEmail.EnviaMensagemEmail(txtEnviarPara.Text,
                        //    txtEnviadoPor.Text, txtAssunto.Text, txtMensagem.Text, txtSenhaRemet.Text);
