...
using DMZ.UI.Annotations;
using DMZ.UI.Classes;
using DMZ.UI.UC;

namespace DMZ.UI.Extensions
{
    public static class ExtensionsHelper
    {
        ...
        public static bool IsNotNull(this DataRow dr)
        {
            if (dr == null)
            {
                return false;
        ...
        public static bool CheckAny(this DataTable table, string campo, bool checado = true)
        {
            bool ret = false;
            if (table.HasRows())
            {
                ret = table.AsEnumerable().Any(x=>x.Field<bool?>(campo).Equals(checado));
            }
            return ret;
        }


        public static DataTable GetCheckedRows(this DataTable table, string campo,bool checado=true)
        {
            DataTable ret=null;
            if (table.HasRows())
            {
                ret = table.AsEnumerable().Where(x => x.Field<bool?>(campo).Equals(checado)).CopyToDataTable();
            }
            return ret;
        }
        ...
        public static DataTable Delete(this DataTable table, string filter,string tabela)
        {
            var dr = table.Select(filter);
            if (dr.Length>0)
            {
                dr.Delete(tabela);
            }
        ...
        public static void Delete(this IEnumerable<DataRow> rows, string tabela)
        {
            foreach (var row in rows)
            {
                var gddd = row[$"{tabela}stamp"].ToString();
                gddd = $" {tabela}stamp ='{gddd}'";
                SQL.SqlCmd($"delete {tabela} where {gddd}");
                row.Delete();
            }
        ...
        internal static void SetStatusValue( this Procura status)
        {
            var dt = SQL.GetGen2DT("select top 1 Codigo,Descricao from status  ");
            if (!(dt?.Rows.Count >= 0)) return;
            status.tb1.Text = dt.Rows[0]["Descricao"].ToString();
            status.Text2 = dt.Rows[0]["Codigo"].ToString();
        }

        internal static void SetPeStatusValue( this Procura status)
        {
            var dt = SQL.GetGen2DT("select codigo,descricao from PeAuxiliar where Tabela=19 and padrao=1");
            if (!dt.HasRows()) return;
            status.tb1.Text = dt.Rows[0]["Descricao"].ToString();
            status.Text2 = dt.Rows[0]["Codigo"].ToString();
        }
        //Set Centro do Custo
        internal static void SetCCustoValue( this Procura status)
        {
            //var dt = SQL.GetGen2DT("select top 1 Codigo,Descricao from status  ");
            //if (!(dt?.Rows.Count >= 0)) return;
            status.tb1.Text = Pbl.Usr.Ccusto;
            status.Text2 = Pbl.Usr.Ccustamp;
        }
